...
class AppRepository(private val context : Context) {
    ...
    fun getDemoIntentBuilder() : RdtIntentBuilder<*> {
        return RdtIntentBuilder.forProvisioning().setSessionId(UUID.randomUUID().toString()) //.requestTestProfile("debug_mal_pf_pv")s
                ...
                .requestProfileCriteria(prefs().getString(PREFERENCE_KEY_DIAGNOSTIC,"hiv1  syph hepB mal_pf mal_pv")!!, ProvisionMode.CRITERIA_SET_OR) //.requestProfileCriteria("sd_bioline_mal_pf_pv carestart_mal_pf_pv", ProvisionMode.CRITERIA_SET_OR)
